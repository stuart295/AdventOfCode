...
def solve_part_1(g, w, h, path, start=(1, 0), cache=None):
    # g.nodes[start]['char'] = 'E'
    # G.graph = g
    # G.draw('char')

    state = to_array(g, w, h)
    ...
    for ox, oy in grid_offsets():
        ...
        if neigh in next_g.nodes and len(next_g.nodes[neigh]['blizz']) == 0:
            sub_path = solve_part_1(next_g, w, h, path.copy(), neigh, cache)
            if not sub_path:
                continue
    ...
    if not final_path:
        ...
        if len(next_g.nodes[stay]['blizz']) == 0:
            sub_path = solve_part_1(next_g, w, h, deepcopy(path), stay, cache)
            if sub_path:
                if len(sub_path) < shortest:
                    final_path = sub_path
...
def solve(lines):
    ...
    start = (1, 0)
    path = solve_part_1(G.graph, G.w, G.h, [], start, cache)
