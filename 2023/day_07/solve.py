...
def solve_part_1(bids, cards, card_val_map):
    clean_hands = [[card_val_map.get(x, x) for x in card] for card in cards]
    ranked = sorted(zip(clean_hands, bids), key=functools.cmp_to_key(compare_func))
    return sum([(i + 1) * bid for i, (_, bid) in enumerate(ranked)])


def solve(lines):
    hands, bids = process_input(lines)

    part1 = solve_part_1(bids, hands, card_map)
    part2 = solve_part_1(bids, hands, card_map_2)
    return part1, part2
